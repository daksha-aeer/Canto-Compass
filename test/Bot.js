...
const getWalletBalance_1 = require("./getWalletBalance");
...
bot.command('analytics', async (ctx) => {
    ...
    bot.action('get_balance', async (ctx) => {
        const addressBal = await (0, getWalletBalance_1.getBalance)(walletAddress);
        ctx.telegram.sendMessage(ctx.chat?.id, `${addressBal}`, {
            reply_markup: {
                inline_keyboard: [
    ...
    bot.action('get_txn_summary', async (ctx) => {
        const transSumm = await (0, getTransSummary_1.getTransactionSummary)(walletAddress);
        ctx.telegram.sendMessage(ctx.chat?.id, `${transSumm}`, {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "Go back to Menu", callback_data: "go" }]
                ]
...
bot.command('queries', (ctx) => {
    ...
    bot.action('canto_blockchain', async (ctx) => {
        ctx.telegram.sendMessage(ctx.chat?.id, 'Canto is a permissionless general-purpose blockchain running the Ethereum Virtual Machine (EVM). It was built to deliver on the promise of DeFi – that through a post-traditional financial movement, new systems will be made accessible, transparent, decentralized, and free.\nLink to the official website - [Canto](https://explore.canto.io/)', {
            reply_markup: {
                inline_keyboard: [
    ...
    bot.action('liquidity', async (ctx) => {
        ctx.telegram.sendMessage(ctx.chat?.id, `The canto.io frontend offers two interfaces for providing liquidity on the Canto DEX and Ambient:\n\n- canto.io/lp – for incentivized pairs\n- pools.canto.io – for all other pairs\n\nFor more information, check out https://docs.canto.io/user-guides/providing-liquidity`, {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "Go back to Menu", callback_data: "go_back" }]
                ]
    ...
    bot.action('assets', async (ctx) => {
        ctx.telegram.sendMessage(ctx.chat?.id, `The Canto Lending Market at canto.io/lending allows users to lend and borrow assets. Currently, USDC, USDT, and NOTE can be supplied to and borrowed from the Canto Lending Market. Additionally, USYC can be supplied.\n\nFor more information, check out the documentation https://docs.canto.io/user-guides/lending-and-borrowing`, {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "Go back to Menu", callback_data: "go_back" }]
                ]
